        ...
        getData($apiURL + '/transglobal/shipment/' + $('#input_shipmentnumber').val())
            .then(response => {
                if (response != "") {
                    ...
                    var html = "";
                    for (var i = 0; i < response.length; i++) {
                        html += "<tr><td class='container-details' data-th='Container details'><span class='icon icon-document  icon--inline-block' aria-hidden='true'></span><span class='screen-reader-only'>document_icon</span><span class='pseudo-header--data-key'>Container details</span><span>" + response[i]['Container Number'] + "</span></td><td class='container-size' data-th='Container type size'><span class='icon icon-container-size  icon--inline-block' aria-hidden='true'></span><span class='screen-reader-only'>container-size_icon</span><span class='pseudo-header--data-key'>Container type size</span><span>" + response[i]['Container Type'] + "</span></td>";
                        if (response[i]['Loading date'] != "")
                            html += "<td class='loading-date' data-th='Loading date'><span class='icon icon-eta  icon--inline-block' aria-hidden='true'></span><span class='screen-reader-only'>eta_icon</span><span class='pseudo-header--data-key'>Loading date</span><span>" + getDate(response[i]['Loading date']) + "</span></td>";
                        html += "<td class='estimated-arrival-date' data-th='Estimated arrival date'><span class='icon icon-eta  icon--inline-block' aria-hidden='true'></span><span class='screen-reader-only'>eta_icon</span><span class='pseudo-header--data-key'>Estimated arrival date</span><span>" + getDate(response[i]['Estimated Arrival Date']) + "</span></td>";
                        if (response[i]['Last Status'] != "")
                            html += "<td class='last-status' data-th='Last Status'><span class='icon icon-location-pin  icon--inline-block' aria-hidden='true'></span><span class='screen-reader-only'>location-pin_icon</span><span class='pseudo-header--data-key'>Last status</span><span>" + response[i]['Last Status'] + "</span></td>";
                        if (response[i]['Loading date'] != "" && response[i]['Last Status'] != "")
                            html += "<td class='expandable-table__expand-row data-table__nested'><div class='timeline' id='timelineId'></div></td><td class='expandable-table__call-to-action'><a href='#' aria-disabled='false' class='button' aria-expanded='false'><span class='icon icon-chevron-down ' aria-hidden='true'></span><span class='screen-reader-only'>chevron-down_icon</span></a></td><td class='loading expandable-table__call-to-action' style='display:none; padding-right: 5px; margin-top: -5px;'><div class='spinner-border text-info' style='width: 2rem; height: 2rem;' role='status'><span class='sr-only'>Loading...</span></div></td><td><input class='input' style='display:none' value='" + response[i]['Container Id'] + "'/></td>";
                    }
